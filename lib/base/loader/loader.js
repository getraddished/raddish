...
BaseLoader.prototype.load = function (identifier) {
    ...
    return new Promise(function (resolve, reject) {
        if (cache[orgIdentifier.toString()]) {
            ...
            console.log('From Cache: ' + orgIdentifier.toString());
            resolve(cache[orgIdentifier.toString()]);
        } else {
            // No cache found, resolve it and cache it.
            self.resolve(identifier, function (Obj) {
                // Set the object in the cache.
                console.log('Setting Cache: ' + orgIdentifier.toString());
...
BaseLoader.prototype.resolve = function (identifier, callback) {
    ...
    async.waterfall([
        ...
        function resolve_step5(next) {
            ...
            identifier.setComponent(path.shift());
