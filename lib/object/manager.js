...
ObjectManager.prototype.getComponentConfig = function(key) {
    ...
    if(!configs[identifier] ) {
        ...
        if(fs.existsSync(component)) {
            config = JSON.parse(fs.readFileSync(component));
        } else {
            config = {};
        }

        if(application) {
            application = JSON.parse(fs.readFileSync(application));

            ObjectManager.extendConfig(config, application);
        }
 ...
 * @private
 */
ObjectManager.extendConfig = function(target, source) {
    for(var index in source) {
        ...
        if(!target[index]) {
            ...
            if(typeof source[index].join === 'function' || typeof source[index] === 'string' || typeof source[index] === 'number') {
                ...
                this.extendConfig(target[index], source[index]);
            }
        }
    }
};
