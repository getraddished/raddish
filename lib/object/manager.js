...
class ObjectManager {
     ...
     */
    clone() {
        return this.getObject(this.getIdentifier(), this.config, true);
    }
     ...
     * @return {Promise} A promise containing the initialized object.
     */
    getObject(identifier, config, refresh) {
        config = config || {};

        var cache = this.validateCache(identifier, config);
        if(cache && cache.clone && refresh === true) {
            return cache.clone();
        }
        ...
        return (typeof object['_initialize'] == 'function' ? object._initialize(config) : Promise.resolve(object))
            .then(function(object) {
                if(refresh !== true) {
                    Loader.addCache(identifier + ':' + getConfigIdentifier(config), object);
                }
                return object;
            });
    }
    ...
    validateCache(identifier, config) {
        return Loader.getCache(identifier + ':' + getConfigIdentifier(config));
    }
