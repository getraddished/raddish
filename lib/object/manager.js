...
/**
 * Abstract execute method that all the objects will now have,
 * however this still has to be implemented in the receiving objects.
 *
 * @method execute
 ...
 */
ObjectManager.prototype.execute = function(method, context) {
    var self = this,
        chain = this.getCommandChain();

    method = method.toLowerCase();

    if(!self['_action' + Inflector.capitalize(method)]) {
        throw new RaddishError(500, 'method:' + method + ' function definition is missing!');
    }
    ...
    return chain.run('initialize.' + method, context)
        ...
        .then(function(data) {
            context.setProperty('result', data);
...
/**
 * If the key is found in the given objects then this will be returned,
 * if not the fallback will be returned.
 *
 * @method getValueFromFallback
 * @param {String} key The key to search for.
 * @param {*} fallback The fallback value.
 * @returns {*} The found value or fallback value.
 */
ObjectManager.prototype.getValueFromFallback = function(key, fallback) {
    var objects = Array.prototype.splice.call(arguments, 2);
