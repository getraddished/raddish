...
ObjectLoader.registerAlias = function(alias, identifier) {
...
ObjectLoader.load = function (identifier, config) {
    ...
    if (cache[[orgIdentifier.toString(), configIdentifier].join('_')]) {
        ...
        return Promise.resolve(obj.clone());
    } else {
        return Promise.resolve(self.prototype.resolve(identifier))
            .then(function (Obj) {
                ...
                var path = obj.getIdentifier().getPath(),
                    key = path[path.length - 1] + '.' + obj.getIdentifier().getName(),
                    component_config = obj.getComponentConfig(key);

                for(var index in component_config) {
                    if(!config[index]) {
                        config[index] = component_config[index];
                    }
                }
