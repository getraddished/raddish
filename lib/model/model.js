    ...
    return Model.super_.prototype.initialize.call(self, config)
        ...
        .then(function (columns) {
            for(var index in columns) {
                self.states.insert(index, 'int', null, true);
            }
...
Model.prototype.getTable = function () {
    ...
    if(self.table !== undefined) {
        return Promise.cast(self.table);
    } else {
        return self.getService(identifier.setPath(['database', 'table']).setName(Inflector.singularize(identifier.getName())))
            .then(function(table) {
                self.table = table;
                return table;
            });
