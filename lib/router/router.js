...
class Router {
    ...
    route(request, response) {
        ...
        if(!route) {
            fs.access(this.publicPath + request.url, fs.R_OK, function(err) {
                if(err) {
                ...
                fs.createReadStream(this.publicPath + request.url).pipe(response);
            }.bind(this));

            return;
        }

        return this.serveFile(request.url, response)
            .catch(function(err) {
    ...
    serveFile(path, response) {
        ...
        path = [this.publicPath, uri.pathname].join('');
