...
Router.prototype.checkFile = function(req, res) {
    ...
    return new Promise(function(resolve, reject) {
        var hosts = Raddish.getConfig('host');
        var host = Router._getHost(req);

        var path = process.cwd() + Raddish.getConfig('public');
        if(host === hosts || hosts === false) {
            // We don't do nested views with a single domain.
            path += '/default';
        } else if(hosts.indexOf(host) > -1) {
            path += '/' + host;
        } else {
            throw new RaddishError(500, 'Domain not recognized');
        }
