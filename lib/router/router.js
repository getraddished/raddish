...
Router.prototype.route = function(req, res) {
    ...
    // First check if the route is configured in the routes config.
    ...
    fs.exists(process.cwd() + Raddish.getConfig('publicFolder') + req.url, function (exists) {
        if (exists) {
...
Router.prototype.parseRequest = function (req) {
    ...
    var parts = req.socket.server._connectionKey.split(':');
    req.port = parts[parts.length - 1];
