    ...
    return this.connection.queryAsync('SHOW FULL COLUMNS FROM ' + name)
        .then(function(rows) {
            ...
            for(var index in rows[0]) {
                var filter = rows[0][index].Comment ? self.getFilter(rows[0][index].Comment.match(/@Filter\("(.*)"\)/)[1]) : undefined;
                ...
                result[rows[0][index].Field] = {
                    ...
                    filter: filter || self.getFilter(self.getType(rows[0][index].Type))
                };
            }
