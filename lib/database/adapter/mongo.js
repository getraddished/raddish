...
MongoAdapter.prototype.execute = function(query) {
    /**
     * This method will see a little change.
     * In cast that it will check the method to execute and call a custom execute metnoh.
     */
 ...
 * @private
 */
MongoAdapter.prototype._executeSelect(collection, query) {
    return new Promise(function(resolve, reject) {
        collection.select(query.toQuery, function(err, result) {
            if(query.getLimit() > 0) {
