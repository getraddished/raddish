...
SluggableBehavior.prototype.onAfterUpdate = function(context) {
    ...
    if(context.getProperty('row').data[this.config.column] && Object.keys(context.getProperty('row').data).indexOf('slug') > -1) {
        var slug = filter.sanitize(context.getProperty('row').data[this.config.column]);

        return context.getProperty('row').getTable()
            .then(function(table) {
            ...
            .spread(function(data, table, query) {
                if(data.rows.length === 0) {
                    context.getProperty('row').setData({slug: slug});
...
SluggableBehavior.prototype.createSlug = function(context, table, query, slug) {
    ...
    return table.select(select_query, 2)
        .then(function(data) {
            ...
            context.getProperty('row').setData({slug: slug + '-' + i});
