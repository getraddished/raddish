...
Table.prototype.initialize = function (config) {
    ...
    this._identity_column = this.getValueFromFallback('identity_column', null, config, extra);
    this._column_map = this.getValueFromFallback('column_map', {}, config);
    this.filters = this.getValueFromFallback('filters', {}, config, extra);
    config.behaviors = this.getValueFromFallback('behaviors', {}, config, extra);

    this.db = this.getValueFromFallback('db', 'default', config, extra);
    ...
    return Abstract.prototype.initialize.call(self, config)
        .then(function () {
...
Table.prototype._afterSelect = function(context) {
    ...
    return object
        .then(function(object) {
            ...
            } else if(context.mode === 2 || context.mode === 3) {
                for(var index in context.result) {
                    if(context.result.hasOwnProperty(index)) {
