...
Table.prototype.insert = function (row) {
    ...
    return this.getQuery()
        ...
        .spread(function(context, columns, adapter) {
            ...
            // We will loop through the columns so no issue here.
            for(var index in columns) {

                // Check if the column is present.
                // Else don't do it. This is because MongoDB makes columns on the fly.
                if(columns[index] && columns[index].filter.validate(context.data.data[index])) {
    ...
    return this.getQuery()
        ...
        .spread(function(context, columns, adapter) {
            ...
            for(var index in context.data.modified) {
                if((adapter.strictColumns ? columns[index] : true)) {
                    if(columns[index] && columns[index].filter.validate(context.data.modified[index])) {
                        context.data.modified[index] = columns[index].filter.sanitize(context.data.modified[index]);
                    }

                    if(columns[index] && !columns[index].autoinc && (adapter.strictColumns ? context.data.data[index] : true)) {
