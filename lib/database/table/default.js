...
Table.prototype.initialize = function (config) {
    ...
    this._db = config.db || 'default'
    this._db_config = this.getConfig('db')[this._db];
    this._adapter = this._db_config.type || 'mysql';
    this.name = (this._db_config.prefix || '') + (config.name || self.getIdentifier().getPackage() + '_' + Inflector.pluralize(self.getIdentifier().getName()));

    if(this._db_config === undefined) {
