 ...
 * @param {Boolean} Force the query in non-stream mode.
 * @returns {Promise} The data requested from the database.
 */
Table.prototype.select = function (query, mode, force) {
    var self        = this;
    ...
    var promise = this.getCommandChain()
        ...
        .spread(function (adapter, context) {
            return adapter.execute(context.query, force);
        });

    if(Raddish.getConfig('stream') && !force) {
        return self._selectStream(promise, context);
    }
...
Table.prototype._selectStream = function(data, context) {
    var self = this;

    return data.then(function(data) {
        return data.pipe(through2.obj(function(chunk, enc, callback) {
            return self.getRow()
                .then(function(row) {
                    row.setData(self.mapColumns(chunk, true));
                    row.new = false;
...
Table.prototype.insert = function (row) {
    ...
    var promise = this.getQuery()
        ...
        .spread(function (context, chain) {
            return [chain.run('initialize.insert', context), chain];
        })
        .spread(function(context, chain) {
    ...
    if(Raddish.getConfig('stream')) {
        return this._insertStream(promise);
    }

    return promise.spread(function(result, context) {
            context.data.data.id = result.insertId;
            context.data.data = self.mapColumns(context.data.data, true);
...
Table.prototype._insertStream = function(promise) {
    var self = this;

    return promise.spread(function(result, context) {
        return result.pipe(through2.obj(function(chunk, enc, callback) {
            return Promise.cast(chunk)
                .then(function(row) {
                    context.data.data.id = result.insertId;
                    context.data.data = self.mapColumns(context.data.data, true);
...
Table.prototype.update = function (row) {
    ...
    var promise = this.getQuery()
        ...
        .spread(function (context, chain) {
            return [chain.run('initialize.update', context), chain];
        })
        .spread(function(context, chain) {
    ...
    if(Raddish.getConfig('stream')) {
        return this._updateStream(promise);
    }
...
Table.prototype._updateStream = function(promise) {
    var self = this;
...
Table.prototype.delete = function (row) {
    ...
    var promise = this.getQuery()
        ...
        .spread(function (adapter, context, chain) {
            return [self.getAdapter(), chain.run('initialize.delete', context), chain];
        })
        .spread(function(adapter, context, chain) {
    ...
    if(Raddish.getConfig('stream')) {
        return this._deleteStream(promise);
    }
...
Table.prototype._deleteStream = function(promise) {
    var self = this;
