...
Table.prototype.select = function (query, mode) {
    ...
    return this.getCommandChain()
        ...
        .then(function (data) {
            if (mode === 1) {
                return self.mapColumns(data, true);
            } else if (mode === 2) {
...
Table.prototype.insert = function (row) {
    ...
    return this.getCommandChain()
        ...
        .spread(function(query, context, chain, columns, adapter) {
            ...
            if(adapter.strictColumns) {
                for(var index in columns) {
                    if(!columns[index].autoinc && context.data.data[index]) {
                        query.set(columns[index].name, context.data.data[index]);
                    }
                }
            } else {
                for(var index in context.data.data) {
                    if(context.data.data[index]) {
                        query.set(index, context.data.data[index]);
                    }
                }
            }
