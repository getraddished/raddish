...
Table.prototype.select = function (query, mode) {
    ...
    return this.getCommandChain()
        .then(function (chain) {
            return [self.isConnected(), chain.run('before.select', context), chain];
        })
        .spread(function (connection, context, chain) {
            localChain = chain;

            console.log(context.query.toQuery());

            return connection.queryAsync(context.query.toQuery());
        })
        .then(function (data) {
            data = data[0];
...
Table.prototype.insert = function (row) {
    ...
    return this.getCommandChain()
        .then(function(chain) {
            return [self.getQuery(), chain.run('before.insert', context), chain, self.getColumns()];
        })
        .spread(function(query, context, chain, columns) {
            var query = query.insert();
            query.into(context.table);

            for(var index in columns) {
                if(!columns[index].unique && context.data.data[index]) {
                    query.set(columns[index].name, context.data.data[index]);
                }
            }

            context.query = query;

            return [self.isConnected(), context, chain];
        })
        .spread(function(connection, context, chain) {
            return [connection.queryAsync(context.query.toString()), chain, context];
        })
        .spread(function(result, chain, context) {
            context.data.data.id = result[0].insertId;
...
Table.prototype.update = function (row) {
    ...
    return this.getCommandChain()
        .then(function(chain) {
            return [self.getQuery(), chain.run('before.update', context), chain, self.getColumns()];
        })
        .spread(function(query, context, chain, columns) {
            ...
            for(var index in columns) {
                if(!columns[index].unique && context.data.modified[index]) {
                    query.set(columns[index].name, context.data.data[index]);
                }
            }
...
Table.prototype.getType = function(type) {
