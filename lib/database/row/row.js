...
Row.prototype.save = function () {
    ...
    return self.table.getQuery()
        .then(function(query) {
            var query = self.new ? query.insert() : query.update();
            self.new ? query.into(self.table.getName()) : query.table(self.table.getName());

            for(var index in self.modified) {
                if(typeof self.data[index] !== 'function' && self.data[index]) {
                    query.set(index, self.data[index]);
                }
            };

            return [self.table.getUniqueColumns(), query];
        })
        .spread(function (columns, query) {
            if(!self.new) {
                for(var index in columns) {
                    query.where(columns[index].name + ' = ?', self.data[columns[index].name]);
                }
            }

            return self.new ? self.table.insert(query) : self.table.update(query);
        });
};
...
Row.prototype.delete = function () {
    ...
    return row.table.getQuery()
        .then(function(query) {
            query = query.delete();
            query.from(row.table.getName());

            return [query, row.table.getUniqueColumns()]
        })
        .spread(function (query, columns) {
            for(var index in columns) {
                query.where(columns[index].name + ' = ?', row.data[index]);
            }

            return row.table.delete(query);
        });
};
