...
var RowAbstract         = require('./abstract'),
    util                = require('util'),
    ObjectIdentifier    = require('../../object/identifier/identifier');
    ...
    return this.getTable()
        .then(function(table) {
            var context = table.getContext()
                .setProperty('row', self);

            if(self.new) {
                return table.execute('insert', context);
            } else {
                return table.execute('update', context);
            }
        });
};
...
Row.prototype.delete = function () {
    ...
    return this.getTable()
        .then(function(table) {
            ...
            return table.execute('delete', context);
        });
};
...
Row.prototype.getTable = function() {
    ...
    if(this.table) {
        if(this.table instanceof ObjectIdentifier) {
            return self.getObject(this.table);
        } else {
