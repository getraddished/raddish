    ...
    return this.getAuthenticator()
        ...
        .then(function(result) {
            var controller = result[0],
                ...
                data = result[1],
                method = (data.fields && data.fields.action ? data.fields.action : req.method);


            context.auth        = auth;
            context.request     = result[2];
            context.data        = result[1];

            return controller.execute(method, context);
        });
};
