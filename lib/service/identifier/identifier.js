...
function ServiceIdentifier(identifier) {
    ...
    var parts = url.parse(identifier);

    if(parts.host.indexOf('.') > -1) {
        parts.pathname = parts.host;
        parts.host = '';
    }

    var _type           = parts.protocol.replace(':', '');
    var _application    = parts.host;
    var _path           = parts.pathname.replace('/', '').split('.');
