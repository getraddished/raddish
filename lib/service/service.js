...
Service.prototype.getService = function (identifier, config) {
    ...
    return ServiceLoader.load(identifier)
 ...
 */
Service.prototype.getComponentConfig = function(identifier, key) {
    var Application = require('../application/application');
    var config = {};

    // Get the config key which will be like <app>:<component>
    if(!key&& identifier) {
        key = identifier;
        identifier = this.getIdentifier().getApplication() + ':' + this.getIdentifier().getComponent();
    }
    ...
    if(!configs[identifier] ) {
        ...
        var app = Application.getApplication(parts[0]);
        if(!app) {
            return false;
        }

        var file = app.config.componentFolder + '/' + parts[1] + '/config.json';
