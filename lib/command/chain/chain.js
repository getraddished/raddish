...
function CommandChain(parent) {
    ...
    return Promise.resolve(this.processBehaviors());
}
    ...
    return Promise.all(functions)
        .then(function(behaviors) {
            ...
            for(var index in behaviors) {
                if(behaviors[index].hasMethod(method)) {
                    result.push(behaviors[index].execute(method, context));
                }
            }
...
CommandChain.prototype.processBehaviors = function () {
    ...
    for(var index in this.behaviors) {
        if (index.indexOf('.') !== -1) {
            this.behaviors[index] = ObjectManager.get(index, this.behaviors[index]);
        } else {
            this.behaviors[index] = ObjectManager.get(identifier.setName(index), this.behaviors[index]);
        }
    }
