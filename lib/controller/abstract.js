...
AbstractController.prototype.initialize = function (config) {
    ...
    return AbstractController.super_.prototype.initialize.call(this, config);
};
    ...
    return this.getModel()
        .then(function(model) {
            if(ObjectLoader.require(identifier.setPath(['view', self.getIdentifier().getName()]).setName(self.format))) {
                return [self.getObject(identifier.setPath(['view', self.getIdentifier().getName()]).setName(self.format)), model];
            }

            return [self.getObject(identifier.setPath(['view']).setName(self.format)), model, self.getRequest()];
        })
        .spread(function(view, model, request) {
            view.model = model;
