    ...
    return self.getPermissions()
        ...
        .then(function(may) {
            if(!may) {
                ...
                return self.getCommandChain().run('initialize.' + method, context);
            }
        })
        .then(function(context) {
            return self.getCommandChain().run('before.' + method, context);
        })
        .then(function(context) {
            return self['_action' + Inflector.capitalize(method.toLowerCase())](context);
        })
        .then(function(context) {
            return self.getCommandChain().run('after.' + method, context);
        })
        .then(function(context) {
            context.clearRoles();
