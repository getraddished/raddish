...
util.inherits(Controller, AbstractController);

/**
 * This method will react on a GET request.
 * This method will check if the view is singular if it is than _actionRead is called, else _actionBrowse is called,
...
Controller.prototype._actionGet = function (context) {
    ...
    return this.getRequest()
        .then(function(request) {
            var method = Inflector.isPlural(request.view) ? 'browse' : 'read';
...
Controller.prototype._actionAdd = function(context) {
    return this.getModel()
        ...
        .then(function(row) {
            return row.setData(context.getProperty('data').fields);
        })
        .then(function(row) {
            return row.save();
        })
...
Controller.prototype._actionEdit = function(context) {
    return this.getModel()
        ...
        .then(function(item) {
            return item.setData(context.getProperty('data').fields);
        })
        .then(function(row) {
            return row.save();
        })
...
/**
 * This method is called on a DELETE method,
